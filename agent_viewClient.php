...
<main id="main">
    <div class="client-list-form">
        ...
        $sql = "SELECT client_id, client_fname, client_lname, client_status FROM clients";
        $result = $pdo->query($sql);

        if ($result) {
            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                $client_id = htmlspecialchars($row["client_id"]);
                $client_name = htmlspecialchars($row["client_fname"] . " " . $row["client_lname"]);
                $client_status = htmlspecialchars($row["client_status"]);
                ?>

                <div class="client-row">
                    <div class="client-info">
                        <strong><?php echo $client_id; ?>.</strong> <?php echo $client_name; ?>
                        <span class="client-status">(<?php echo $client_status; ?>)</span>
                    </div>
            ...
            echo "<p style='text-align: center; color: red;'>Error fetching client data.</p>";
        }
